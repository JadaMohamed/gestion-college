...
public class SallesPaneController {
    ...
    // CategorieSalle selectedCategory = categoryComboBox.getValue(); // Obtenir la
    // catégorie de salle sélectionnée

    // // Filtrer les données des salles en fonction de la catégorie de salle
    // sélectionnée
    // Predicate<Map<String, String>> sallePredicate = salle -> {
    // String salleCategory = salle.get("categorieSalle"); // Remplacez
    // "categorieSalle" par le nom de la colonne correspondant à la catégorie de
    // salle dans vos données
    // return salleCategory.equals(selectedCategory.toString()); // Vérifiez si la
    // catégorie de salle de la salle correspond à la catégorie sélectionnée
    // };

    // // Appliquer le prédicat de filtre et afficher les résultats dans la table
    // des salles
    // ObservableList<Map<String, String>> filteredData =
    // data.filtered(sallePredicate);
    ...
    public void search(String keyword, TableView<Map<String, String>> sallesTableView) {
        ...
        // TableView<Map<String, String>> tableView = sallesTableView; // assuming
        // sallesTableView is accessible
        ...
        // return nomSalle.contains(keyword.toLowerCase()); // Vérifiez si le nom de la
        // salle contient le mot-clé de recherche
        // };
    ...
    public void setActiveSalleInformation(Map<String, String> rowData, Text activeSalleLabel,
        ...
        if (Integer.parseInt(rowData.get("statut")) == 1) {
            activeSalleStatutIcon.setImage(
                    new Image(getClass().getResourceAsStream("/resources/images/icons/Badge_Disponible.png")));
        } else {
            activeSalleStatutIcon
                    .setImage(new Image(getClass().getResourceAsStream("/resources/images/icons/Badge_Occupée.png")));
        }
